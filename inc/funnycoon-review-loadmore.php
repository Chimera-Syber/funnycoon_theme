...
function funnycoon_review_loadmore_ajax_handler( $initial_request = false ) {
    ...
    if ( $query->have_posts()  ):
		...
		while ( $query->have_posts() ): $query->the_post();
			if ($count == 1): 
				get_template_part('template-parts/content/content-main-reviews-slider-card-1');
			else: 
				get_template_part('template-parts/content/content-main-reviews-slider-card-2');
			endif;
			$count++;
		endwhile;
